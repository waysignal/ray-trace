...
fn main() {
    ...
    left.transform = translation(-1.1, 0.33, 0.15).dot(scale(0.33,0.33,0.33)).unwrap();
    ...
    let v = [cone,cube,left,right,middle,g]; //cone,cube,left,right,middle,
    for (_a,b) in v.iter().enumerate(){
        add_child(&mut g1_rc, b);
    }
    let v = [p,right_wall,left_wall];
    for (_a, b) in v.iter().enumerate(){
    ...
    let world = World{ light_source: PointLight::new(point(-10.0,10.0,-10.0),Color::new(1.0,1.0,1.0)),
                        objects: vec![g3_rc,g2_rc]};//,g2_rc.borrow().clone() 

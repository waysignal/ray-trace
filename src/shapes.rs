...
-> Rc<RefCell<Box<dyn ShapeThings>>>
    ...
    //can unwrap the parent of cylinders here
    side_rc
...
impl BoundingBox {
    ...
    pub fn new(given_min: Element, given_max: Element) 
    -> BoundingBox{
    ...
    pub fn add_in(self, add_point: Element) 
    -> BoundingBox{
        
        let [min_x, min_y, min_z] = [self.min.x(),self.min.y(),self.min.z()];
        let [max_x, max_y, max_z] = [self.max.x(),self.max.y(),self.max.z()];
        let [new_x, new_y, new_z] = [add_point.x(), add_point.y(), add_point.z()];


        BoundingBox{
            min: point(min_x.min(new_x),min_y.min(new_y),min_z.min(new_z)),
            max: point(max_x.max(new_x),max_y.max(new_y),max_z.max(new_z)),
        }
    }

    pub fn merge_in_box(self, other: BoundingBox) 
    -> BoundingBox{
        let min_box = self.add_in(other.min);
        min_box.add_in(other.max)
    }

    pub fn contains_box(&self, other: &BoundingBox) 
    -> bool{
        self.min.x() < other.min.x() && other.min.x() < self.max.x() &&
        self.min.y() < other.min.y() && other.min.y() < self.max.y() &&
        self.min.z() < other.min.z() && other.min.z() < self.max.z() 
    }

    pub fn contains_point(&self, p: Element)
    -> bool{
    ...
    -> BoundingBox{
        ...
        let mut points = vec![];
        for x in [self.max.x(),self.min.x()]{
            for y in [self.max.y(),self.min.y()]{
                for z in [self.max.z(),self.min.z()]{
                    points.push(point(x,y,z));
                }
            }
        }
        for p in points{
            result = result.add_in(Element{matrix: m.dot(p.matrix).unwrap()});
        }
        result
    }
...
impl Pattern for CheckersPattern{
        ...
        fn pattern_at_shape(&self, obj: Rc<RefCell<Box<dyn ShapeThings>>>, world_point: Element) -> Color  {
            let obj_point = world_to_object(&obj, &world_point).matrix;
...
impl Pattern for StripePattern{
    ...
    fn pattern_at_shape(&self, obj: Rc<RefCell<Box<dyn ShapeThings>>>, world_point: Element) -> Color  {
        let obj_point = world_to_object(&obj, &world_point).matrix;//obj.get_transform().invert().unwrap().dot(world_point.matrix).unwrap();
        let pattern_point = Element{ matrix: self.get_transform().invert().unwrap().dot(obj_point).unwrap()};
        self.pattern_at(pattern_point)
...
impl ShapeThings for Group{ 
    ...
    // }
    //RefCell<Vec<RefCell<Box<dyn ShapeThings>>>>
    fn bounding_box(&self) -> BoundingBox{
        let members = self.get_members().borrow();
        let mut bbox = BoundingBox::empty();
        if members.is_empty(){ return bbox}
        
        for (_i,e) in members.iter().enumerate(){
            bbox = bbox.merge_in_box(e.borrow().bounding_box());
...
pub fn normal_to_world(s: &RefCell<Box<dyn ShapeThings>>, normal: &Element) 
-> Element{
    let normal = normal.clone();
    let mut normal = s.borrow().get_transform().invert().unwrap().transpose().dot(normal.matrix).unwrap();
    normal.matrix[3][0] = 0.0;
    let mut normal = Element{ matrix: normal}.normal();

    if s.borrow().has_parent(){

        normal = normal_to_world(&s.borrow().get_parent().upgrade().unwrap(), &normal);
...
pub fn wrap_this<T> (this: T) -> Rc<RefCell<Box<dyn ShapeThings>>>
    where T: ShapeThings{
    let this = Box::new(this) as Box<dyn ShapeThings>;
    Rc::new(RefCell::new(this))
}



pub fn take_members_out(g: & Rc<RefCell<Box<dyn ShapeThings>>>) -> Vec<Box<dyn ShapeThings>>{

    let mut list = vec![];
    let parent = &**g;
    let parent_members = parent.borrow().get_members().clone();
    
    
 
    //let g = g.members;
    for (_i,e) in parent_members.borrow().iter().enumerate(){
        let e = e.clone();
        let mut j = e.into_inner();
        let matrix = parent.borrow().get_transform().dot(j.get_transform()).unwrap();
...
// pub fn take_members_out_2(g: & Box<dyn ShapeThings>,r: & Ray, mut list: Vec<Box<dyn ShapeThings>>) ->  Vec<Box<dyn ShapeThings>>{
   
//     let local_ray = r.transform(&g.get_transform().invert().unwrap());
//     let parent = g;
//     let parent_members = parent.get_members().clone();
...
//         let e = e.clone();
//         let mut j = e.into_inner();
//         let matrix = parent.get_transform().dot(j.get_transform()).unwrap();
//         j.set_transform(matrix);
//         // let j = j.set_transform(matrix); error: cannot move a value of type dyn shapes::ShapeThings: the size of dyn shapes::ShapeThings cannot be statically determined
//         // function consumes j of type dyn shapes::ShapeThings which cannot be determined bc it is dynamic
//         list.push(j);
...
//         results = results.adder(e.make(xs));
//         eprintln!("results");
//     }
//     return list
// }
...
impl ShapeThings for Plane{
    ...
    -> BoundingBox{
        ...
        let bbox = BoundingBox::new(
            point(NEG_INFINITY, 0.0, NEG_INFINITY),
            point(INFINITY, 0.0, INFINITY)
        );
        bbox.transform(self.get_transform())
    }
    fn get_members(&self) -> &RefCell<Vec<RefCell<Box<dyn ShapeThings>>>> {
...
pub fn check_axis(origin: f64, direction: f64, min: f64, max: f64) -> [f64;2] {
...
impl ShapeThings for Cylinder{
    ...
    -> BoundingBox{
        let bbox = BoundingBox::new(
            point(-1.0, self.min, -1.0),
            point(1.0, self.max, 1.0),
        );
        bbox.transform(self.get_transform())
    }
    fn get_members(&self) -> &RefCell<Vec<RefCell<Box<dyn ShapeThings>>>> {
...
impl ShapeThings for Cone{
    fn bounding_box(&self) 
    -> BoundingBox{
        let limit = self.min.abs().max(self.max.abs());

        let bbox = BoundingBox::new(
            point(-limit, self.min, -limit),
            point(limit, self.max, limit),
        );
        bbox.transform(self.get_transform())
    }
    fn get_members(&self) -> &RefCell<Vec<RefCell<Box<dyn ShapeThings>>>> {
...
pub fn lighting(m: Material, object: &Box<dyn ShapeThings>, light:&PointLight,position: Element,eyev: Element,normalv:Element,in_shadow:bool) -> Color  {
    ...
    if m.pattern.is_some(){
        color = m.pattern.unwrap().pattern_at_shape(Rc::new(RefCell::new(object.clone())), position.clone()); // CHECK
    }
...
impl World{
    pub fn new() -> World {
        ...
        World { objects: vec![(s1).this_is(), (s2).this_is()] , light_source: PointLight::new(point(-10.0,10.0,-10.0), Color::new(1.0,1.0,1.0)) }
    }
...
pub fn intersect_world(w: & World,r:& Ray,mut l:  Vec<Intersection>) -> Intersections{
    ...
    for (_i, j) in w.objects.iter().enumerate(){
        ...
        //let k= j.into_inner();
        // let mut v;
        // if k.get_kind() == Shapes::Group && k.intersect(r).is_empty(){
        //     break;
        // }
        // if k.get_kind() == Shapes::Group {
